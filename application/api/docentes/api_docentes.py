...
# http://0.0.0.0:8080/api_docentes?user_hash=12345&action=put&id_docente=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=0
    def put(self, nom_doc,especialidad,telefono,direccion):
        try:
            config.model.insert_docentes(nom_doc,especialidad,telefono,direccion)
            docentes_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(docentes_json)
        except Exception as e:
...
# http://0.0.0.0:8080/api_docentes?user_hash=12345&action=update&id_docente=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_docente, nom_doc,especialidad,telefono,direccion):
        try:
            config.model.edit_docentes(id_docente,nom_doc,especialidad,telefono,direccion)
            docentes_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(docentes_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.put(nom_doc,especialidad,telefono,direccion)
                elif action == 'delete':
                    return self.delete(id_docente)
                elif action == 'update':
                    return self.update(id_docente, nom_doc,especialidad,telefono,direccion)
            else:
                raise web.seeother('/404')
        except Exception as e:
